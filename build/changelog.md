...
## 2026-07-28 — 工作流程增补“推送远端”要求

**类型**：文档（docs）

**变更内容**：在 `build/README.md` 的「工作流程」新增第 6 步——每次改动后必须 `git push origin main`，且在 changelog 该条「变更内容」中注明改了哪些文件、改了什么。本地未推送的改动不算完成。同时本次将上一条 learn 文章还原提交已推送至 `origin/main`（commit `655034e`）。

**构建验证**：纯文档改动，`npm run build` 无需验证。
